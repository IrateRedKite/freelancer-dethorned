duration  =  299.997;

entities  = 
{

	{
		entity_name  =  "Layer_Br_02_Equip_ambient_int",
		type  =  SCENE,
		template_name  =  "",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { 0, 0, 0 },
			orient  =  { { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 } }
		},
		up  =  Y_AXIS,
		front  =  Z_AXIS,
		ambient  =  { 0, 0, 0 }
	},

	{
		entity_name  =  "gf_rtc_shipcushion_7",
		type  =  PSYS,
		template_name  =  "gf_rtc_shipcushion",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		flags  =  LIT_DYNAMIC + LIT_AMBIENT,
		spatialprops  = 
		{
			pos  =  { 0, 0.009207, 0 },
			orient  =  { { 0.006113, 0.000000, -0.999981 },
					   { 0.000000, 1.000000, 0.000000 },
					   { 0.999981, 0.000000, 0.006113 } }
		},
		psysprops  = 
		{
			sparam  =  0
		}
	},

	{
		entity_name  =  "X/Shipcentre/01",
		type  =  MARKER,
		template_name  =  "",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		flags  =  REFERENCE,
		spatialprops  = 
		{
			pos  =  { -0.081614, -0.169128, -0.26003 },
			orient  =  { { -0.653745, 0.000000, -0.756715 },
					   { 0.000000, 1.000000, 0.000000 },
					   { 0.756715, 0.000000, -0.653745 } }
		}
	},

	{
		entity_name  =  "Marker_Shipcushion",
		type  =  MARKER,
		template_name  =  "",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { 0, 0.009207, -1e-006 },
			orient  =  { { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 } }
		}
	},

	{
		entity_name  =  "AMBI_LT3_PT01",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  3, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { -0.599005, 14.42587, 9.946732 },
			orient  =  { { 1.000000, 0.000000, 0.000000 },
					   { 0.000000, -0.000804, 1.000000 },
					   { 0.000000, -1.000000, -0.000804 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 182, 206, 231 },
			diffuse  =  { 0.662745, 0.572549, 0.662745 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0.043137, 0.043137, 0.043137 },
			direction  =  { 0, 0, 1 },
			range  =  60,
			cutoff  =  60,
			type  =  L_POINT,
			theta  =  49.99999,
			atten  =  { 0.5, 0, 0.0009999999 }
		}
	},

	{
		entity_name  =  "AMBI_LT9_PT01",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  9, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { 5.037045, -13.89192, 0.685075 },
			orient  =  { { 0.067395, -0.074946, 0.994907 },
					   { 0.991429, 0.116893, -0.058354 },
					   { -0.111924, 0.990313, 0.082182 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 174, 232, 249 },
			diffuse  =  { 0.580392, 0.345098, 0.654902 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0.043137, 0.043137, 0.066667 },
			direction  =  { 0, 0, 1 },
			range  =  90,
			cutoff  =  90,
			type  =  L_POINT,
			theta  =  69.99999,
			atten  =  { 0.8, 0, 0.0009999999 }
		}
	},

	{
		entity_name  =  "AMBI_LT9_PT02",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  9, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { -0.599005, 67.5596, 9.989441 },
			orient  =  { { 1.000000, 0.000000, 0.000000 },
					   { 0.000000, -0.000804, 1.000000 },
					   { 0.000000, -1.000000, -0.000804 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 23, 142, 17 },
			diffuse  =  { 0.882353, 0.729412, 0.635294 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0, 0, 0 },
			direction  =  { 0, 0, 1 },
			range  =  80,
			cutoff  =  90,
			type  =  L_POINT,
			theta  =  65,
			atten  =  { 0.5, 0, 0.0001 }
		}
	},

	{
		entity_name  =  "AMBI_LT3_PT02",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  3, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { -4.523118, -15.01913, 1.247893 },
			orient  =  { { 0.067395, -0.074946, 0.994907 },
					   { 0.991429, 0.116893, -0.058354 },
					   { -0.111924, 0.990313, 0.082182 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 174, 232, 249 },
			diffuse  =  { 0.509804, 0.454902, 0.572549 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0, 0, 0 },
			direction  =  { 0, 0, 1 },
			range  =  90,
			cutoff  =  90,
			type  =  L_POINT,
			theta  =  69.99999,
			atten  =  { 1, 0, 0.0009999999 }
		}
	},

	{
		entity_name  =  "AMBI_LT9_PT03",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  9, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { -3.614491, -70.00447, -4.76651 },
			orient  =  { { 0.067395, -0.074946, 0.994908 },
					   { 0.996576, -0.042806, -0.070733 },
					   { 0.047889, 0.996268, 0.071805 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 174, 232, 249 },
			diffuse  =  { 0.894118, 0.760784, 0.87451 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0, 0, 0 },
			direction  =  { 0, 0, 1 },
			range  =  90,
			cutoff  =  90,
			type  =  L_POINT,
			theta  =  69.99999,
			atten  =  { 0.8, 0, 0.0001 }
		}
	},

	{
		entity_name  =  "AMBI_LT0_PT01",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { -2.021386, 2.020638, 21.51877 },
			orient  =  { { -0.080150, 0.036018, -0.996132 },
					   { 0.996781, 0.005056, -0.080019 },
					   { 0.002155, -0.999338, -0.036308 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 197, 222, 192 },
			diffuse  =  { 0.976471, 0.956863, 0.976471 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0.082353, 0.082353, 0.082353 },
			direction  =  { 0, 0, 1 },
			range  =  3,
			cutoff  =  5,
			type  =  L_POINT,
			theta  =  4,
			atten  =  { 0.8, 0, 1 }
		}
	},

	{
		entity_name  =  "AMBI_LT0_PT02",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { -1.334026, 1.92735, 20.71458 },
			orient  =  { { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 205, 193, 156 },
			diffuse  =  { 0.960784, 0.945098, 0.972549 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0.07451, 0.070588, 0.094118 },
			direction  =  { 0, 0, 1 },
			range  =  7,
			cutoff  =  180,
			type  =  L_POINT,
			theta  =  180,
			atten  =  { 1, 0, 0.3 }
		}
	},

	{
		entity_name  =  "AMBI_LT0_PT03",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { 10.60648, 1.512074, 7.3461 },
			orient  =  { { -0.995819, 0.087293, 0.026919 },
					   { 0.035218, 0.094963, 0.994858 },
					   { 0.084288, 0.991646, -0.097640 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 205, 193, 156 },
			diffuse  =  { 0.827451, 0.760784, 0.827451 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0.082353, 0.066667, 0.086275 },
			direction  =  { 0, 0, 1 },
			range  =  3,
			cutoff  =  4,
			type  =  L_POINT,
			theta  =  3,
			atten  =  { 1, 0, 1.25 }
		}
	},

	{
		entity_name  =  "AMBI_LT0_PT04",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { 11.61294, 1.871268, 7.858326 },
			orient  =  { { -0.995819, 0.087293, 0.026919 },
					   { 0.035218, 0.094963, 0.994858 },
					   { 0.084288, 0.991646, -0.097640 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 205, 193, 156 },
			diffuse  =  { 0.580392, 0.654902, 0.686275 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0.082353, 0.066667, 0.086275 },
			direction  =  { 0, 0, 1 },
			range  =  3,
			cutoff  =  4,
			type  =  L_POINT,
			theta  =  3,
			atten  =  { 1, 0, 1.25 }
		}
	},

	{
		entity_name  =  "AMBI_LT0_PT05",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { 4.908005, 2.065665, 10.60344 },
			orient  =  { { -0.995819, 0.087293, 0.026919 },
					   { 0.035218, 0.094963, 0.994858 },
					   { 0.084288, 0.991646, -0.097640 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 205, 193, 156 },
			diffuse  =  { 0.745098, 0.827451, 0.803922 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0.082353, 0.066667, 0.086275 },
			direction  =  { 0, 0, 1 },
			range  =  3,
			cutoff  =  4,
			type  =  L_POINT,
			theta  =  3,
			atten  =  { 1, 0, 0.5 }
		}
	},

	{
		entity_name  =  "AMBI_LT0_PT06",
		type  =  LIGHT,
		template_name  =  "",
		lt_grp  =  0, srt_grp  =  0, usr_flg  =  0,
		spatialprops  = 
		{
			pos  =  { 6.976425, 2.201485, 11.45879 },
			orient  =  { { -0.995819, 0.087293, 0.026919 },
					   { 0.035218, 0.094963, 0.994858 },
					   { 0.084288, 0.991646, -0.097640 } }
		},
		lightprops  = 
		{
			on  =  Y,
			color  =  { 205, 193, 156 },
			diffuse  =  { 0.439216, 0.309804, 0.517647 },
			specular  =  { 0, 0, 0 },
			ambient  =  { 0.082353, 0.066667, 0.086275 },
			direction  =  { 0, 0, 1 },
			range  =  3,
			cutoff  =  4,
			type  =  L_POINT,
			theta  =  3,
			atten  =  { 1, 0, 0.5 }
		}
	}
};

events  = 
{
	{
		0.000, START_PSYS, { "gf_rtc_shipcushion_7" },
		{
			duration  =  300.000
		}
	},

	{
		0.000, ATTACH_ENTITY, { "gf_rtc_shipcushion_7", "Marker_Shipcushion" },
		{
			duration  =  300.000,
			offset  =  { 0, 0, 0 },
			up  =  Y_AXIS,
			front  =  NEG_Z_AXIS,
			target_part  =  "",
			target_type  =  ROOT,
			flags  =  POSITION + ORIENTATION
		}
	},

	{
		0.000, START_FOG_PROP_ANIM, { "Layer_Br_02_Equip_ambient_int" },
		{
			duration  =  0.000,
			fogprops  = 
			{
				fogon  =  Y,
				fogtable  =  N,
				fogcolor  =  { 57, 50, 34 },
				fogmode  =  F_LINEAR,
				fogstart  =  10,
				fogend  =  200,
				fogdensity  =  0.1
			}
		}
	}
};
